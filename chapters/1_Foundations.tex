...
\ex{1.1}
    \begin{enumerate}
        \item 
        $R = 5\k + 10\k = \sol{15\k\Omega}$

        \item 
        $R = \dfrac{R_1 R_2}{R_1 + R_2} = \dfrac{5\k \cdot 10\k}{5\k + 10\k} = \sol{3.33\k\Omega}$

    \end{enumerate}

\ex{1.2}
    $P = IV = \left(\dfrac{V}{R}\right)V = \dfrac{(12\V)^2}{1\Ohm} = \sol{144\W}$

\ex{1.3}
    Consider a simple series resistor circuit.
    \begin{schematic}{fig:1.3.1}{A basic series circuit.}
	...
	where \[\sol{R = R_{1} + R_{2}}\] is the resistance of $R_{1}$ and $R_{2}$ in series. Now, consider a simple parallel resistor circuit.
	
	\begin{schematic}{fig:1.3.2}{A basic parallel circuit.}
	...
	where \[\sol{R = \dfrac{1}{\frac{1}{R_{1}}+\frac{1}{R_{2}}} = \frac{R_{1}R_{2}}{R_{1}+R_{2}}}\] is the resistance of $R_{1}$ and $R_{2}$ in parallel.
\ex{1.4}
	...
	where we have proved that equality holds for $n = k+1$. Finally, the resistance of $n$ resistors in parallel is given by \[\sol{R_{1\cdots n} = \dfrac{1}{\sum_{i=1}^{n}\frac{1}{R_{i}}} = \frac{1}{\frac{1}{R_{1}}+\frac{1}{R_{2}}+\ldots+\frac{1}{R_{n}}}}\]
    \ex{1.5}
    Given that $P = \dfrac{V^2}{R}$, we know that the maximum voltage we can achieve is 15V and the smallest resistance we can have across the resistor in question is $1\k\Ohm$. Therefore, the maximum amount of power dissipated can be given by \[P = \frac{V^2}{R} = \frac{(15\V)^2}{1\k\Ohm} = \sol{0.225\W}\]
    This is less than the 1/4W power rating.

\ex{1.6}
    \begin{enumerate}
        \item
        The total current required by New York City that will flow through the cable is $I = \frac{P}{V} = \frac{10^{10}\;\W}{115\;\V} = 86.96\;\M\A$. Therefore, the total power lost per foot of cable can be calculated by:
        \[P = I^2R = (86.96 \times 10^6\;\A)^2 \times \left(5\times10^{-8}\;\frac{\Ohm}{\ensuremath{\text{ft}}}\right) = \sol{3.78\times10^{8} \frac{\W}{\ensuremath{\text{ft}}}} \] 
        \item
        The length of cable over which all $10^{10}\;\W$ will be lost is:
        \[L = \frac{10^{10}\;\W}{3.78 \times 10^{8}\;\frac{\W}{\ensuremath{\text{ft}}}} = \sol{26.45\;ft}\]
        \item
        ...
        Therefore,
        \[T = \sqrt[4]{\frac{P}{A\sigma}} = \sqrt[4]{\frac{10^{10}\;\W}{7.72 \times 10^4\;{\ensuremath{\text{cm}}}^2 \times 6 \times 10^{-12}\;\frac{\W}{\text{K}^4\text{cm}^2}}} = \sol{12,121\;K} \]
...
\ex{1.7}
	...
	\begin{enumerate}
		...
		\begin{schematic}{fig:1.7.1}{A voltage source with internal resistance and a $20,000 \Ohm/\V$ meter read in its $1 \V$ scale.}
			...
			\draw (5,0) to (0,0)
		\end{schematic}
		Then, we have that the current in the ideal ammeter and the voltage in the meter resistance are given by\footnote{When a meter only measures currents, it puts a resistance in series to measure the current through that resistance and internally converts that current into voltage to \textit{measure voltages}.}
		\[I = \frac{1 \V}{10\k\Ohm + 20\k\Ohm} = \sol{0.0333 \m\A} \quad \text{ and } \quad V = 0.0333 \m\A \cdot 20\k\Ohm = \sol{0.666 \V}\]
		\item In the second question, we have the following circuit:
		\begin{schematic}{fig:1.7.2}{A $10\k\Ohm-10\k\Ohm$ voltage divider and a $20,000 \Ohm/\V$ meter read in its $1 \V$ scale.}
			(0,0) to[V=$1 \V$,invert] (0,4)
			to[short] (3,4)
			to[R=$10 \k\Ohm$] (3,2)
			to[R=$10 \k\Ohm$] (3,0);
		...
		\begin{schematic}{fig:1.7.3}{Thévenin equivalent circuit of circuit in Figure \ref{fig:1.7.2}.}
		...
		\[I = \frac{0.5 \V}{5\k\Ohm + 20\k\Ohm} = \sol{0.02 \m\A} \quad \text{ and } \quad V = 0.02 \m\A \cdot 20\k\Ohm = \sol{0.4 \V}\]
	\end{enumerate}
	
\ex{1.8}
	\begin{enumerate}
		\item In the first part, we have the following circuit:
		\begin{schematic}{fig:1.8.1}{50\u A ammeter with 5k\Ohm\ internal
		...
		Then we simply do
		\[R_s = \frac{V}{I_s} = \frac{0.25 \V}{0.999950 \A} = \sol{0.25 \Ohm}\]
		
		\item In the second part, we have the following circuit:
		\begin{schematic}{fig:1.8.2}{50\u A ammeter with 5k\Ohm\ internal
		...
		\[R_s = \frac{10\V}{50\mu\A} - 5\k\Ohm = \sol{195\k\Ohm}\]
	\end{enumerate}	
...
\ex{1.10}
    \begin{enumerate}
        ...
        \[V_{out} = \frac{1}{2}V_{in} = \frac{30\V}{2} = \sol{15\V}\]
        ...
        \[V_{out} = V_{in} \frac{5\k\Ohm}{10\k\Ohm + 5\k\Ohm} = \frac{30\V}{3} = \sol{10\V} \]
        \todo{Add a diagram to make this clearer}

        \item 
        We can redraw the voltage divider circuit to make the ``port'' clearer. 
        \begin{schematic}{fig:1.10.1}{Voltage divider with port shown.}
            % \label{1.10fig1}
            (0,2) to[V=$V_{\in}$] (0,0)
            to[short] (2,0)
            to[R=$R_2$] (2,2)
        ...
        We can find $V_{\Th}$ by leaving the ports open (open circuit) and measuring $V_\out$, the voltage across $R_2$. This comes out to be half the input voltage when $R_1 = R_2$, so $V_\out = 15\V$. Thus $V_{\Th} = \sol{15\V}$.
        ...
        \begin{schematic}{fig:1.10.2}{Voltage divider with short circuit on the output.}
            (0,2) to[V=$V_{\in}$] (0,0) 
            to[short] (2,0)
            to[R=$R_2$] (2,2)
            to[R=$R_1$](0,2)
        ...
        \[R_\Th = \frac{V_\Th}{I_{SC}} = \frac{V_\Th}{V_\in/R_1} = \frac{15\V}{30\V/10\k\Ohm} = \sol{5\k\Ohm}\]

        The Th\'evenin equivalent circuit takes the form shown below.
        \begin{schematic}{fig:1.10.3}{Th\'evenin equivalent circuit.}
            (0,2) to[V=$V_{\Th}$] (0,0)
            to[short, -o] (3,0)
        ...
        \begin{schematic}{fig:1.10.4}{Th\'evenin equivalent circuit with $10\k\Ohm$ load.}
            (0,2) to[V=$V_{\Th}$] (0,0)
            to[short] (3,0)
            (0,2) to[R=$R_{\Th}$] (3,2)
        ...
        \[V_\out = \frac{10\k\Ohm}{R_\Th + 10\k\Ohm} V_\Th = \frac{10\k\Ohm}{5\k\Ohm + 10\k\Ohm} \cdot 15\V = \sol{10\V}\] 
        ...
        \begin{schematic}{fig:1.10.5}{Original voltage divider with $10\k\Ohm$ load attached.}
        ...
        \[P_\load = \frac{V^2}{R_\load} = \frac{(10\V)^2}{10\k\Ohm} = \sol{10\m\W}\]
        ...
        \[P_1 = \frac{V^2}{R_1} = \frac{(20\V)^2}{10\k\Ohm} = \sol{40\m\W}\]
    \end{enumerate}

\ex{1.11}
    Consider the following Th\'evenin circuit where $R_\source$ is just another name for the Th\'evenin resistance, $R_\Th$.
    \begin{schematic}{fig:1.11.1}{Standard Th\'evenin circuit with attached load.}
    ...
    \begin{align*}
        \frac{dP_\load}{dR_\load} &= V_\Th^2 \frac{(R_\source + R_\load)^2 - 2R_\load(R_\source + R_\load)}{(R_\source + R_\load)^4} = 0 \\
...
\ex{1.12}
    \begin{enumerate}
        \item 
        Voltage ratio: $\frac{V_2}{V_1} = 10^{db/20} = 10^{3/20} = \sol{1.413}$

        Power ratio: $\frac{P_2}{P_1} = 10^{db/10} = 10^{3/10} = \sol{1.995}$

        \item 
        Voltage ratio: $\frac{V_2}{V_1} = 10^{db/20} = 10^{6/20} = \sol{1.995}$

        Power ratio: $\frac{P_2}{P_1} = 10^{db/10} = 10^{6/10} = \sol{3.981}$
        
        \item 
        Voltage ratio: $\frac{V_2}{V_1} = 10^{db/20} = 10^{10/20} = \sol{3.162}$

        Power ratio: $\frac{P_2}{P_1} = 10^{db/10} = 10^{10/10} = \sol{10}$
        
        \item 
        Voltage ratio: $\frac{V_2}{V_1} = 10^{db/20} = 10^{20/20} = \sol{10}$

        Power ratio: $\frac{P_2}{P_1} = 10^{db/10} = 10^{20/10} = \sol{100}$
    \end{enumerate}
...
\ex{1.15}
    ...
    \begin{schematic}{fig:1.15.1}{Two capacitors in series.}
        (0,0) to[C=$C_1$] (2,0)
            to[C=$C_2$] (4,0)
...
\ex{1.17}
    ...
    \begin{schematic}{fig:1.17.1}{Th\'evenin equivalent circuit to Figure 1.36 from the textbook.}
    ...
    \[V(t) = V_\text{final}(1 - e^{-t/RC}) = V_\Th (1 - e^{-t/R_\Th C} = \sol{\frac{1}{2}V_\in (1 - e^{-t/5 \times 10^{-4}})}\]
...
\ex{1.20}
    ...
    \[C \ge \frac{10\m\A}{2\times 60\Hz \times 0.1\V} = \sol{833 \u\F}\]
    ...
    \[V_{\in, \pp} = 10\V + 2(0.6\V) = \sol{11.2\V}\]
